...
json_file = "/home/deck/homebrew/settings/decky-cloud-save/settings.json"

# Common remote directory
remote_dir = "/home/deck/homebrew/data/decky-cloud-save"
...
def process_entry(entry_name, inclusions, exclusions, dest_folder, src_folder):
    ...
    decky_plugin.logger.info(f"Processing {entry_name}")

    # Process inclusions
    files_to_copy = set()
    for inclusion_pattern in inclusions:
        matched_files = find_files(src_folder, inclusion_pattern)
    ...
    for root, dirs, files in os.walk(dest_folder, topdown=False):
        for name in files:
            ...
            relative_path = get_relative_path(dest_file, dest_folder)
            src_file = os.path.join(src_folder, relative_path)

            if not os.path.exists(src_file) and should_delete_file(relative_path, inclusions, exclusions):
                os.remove(dest_file)
                deleted_files += 1
        ...
        for name in dirs:
            ...
            relative_path = get_relative_path(dest_dir, dest_folder)
    ...
    for file in files_to_copy:
        relative_path = get_relative_path(file, src_folder)
        dest_file = os.path.join(dest_folder, relative_path)

        if os.path.exists(dest_file):
            if get_modification_time(file) > get_modification_time(dest_file):
                copy_with_timestamps(file, dest_file)
                modified_files += 1
                total_size += get_file_size(file)  # Count size only if the file is modified
                log_entries.append((relative_path, "(m)"))
        else:
            os.makedirs(os.path.dirname(dest_file), exist_ok=True)
            copy_with_timestamps(file, dest_file)
            created_files += 1
            total_size += get_file_size(file)  # Count size only if the file is created
            log_entries.append((relative_path, "(+)"))

    # Sort log entries alphabetically by relative path, ignoring case
    log_entries.sort(key=lambda x: x[0].casefold())

    # decky_plugin.logger.info log entries for the current entry
    for relative_path, action in log_entries:
        decky_plugin.logger.info(f"    {action} '{relative_path}'")
...
def copyToRemote():
    ...
    for entry_name, details in data.items():
        ...
        c, m, d, size = process_entry(entry_name, inclusions, exclusions, remote_dir + "/" + entry_name, folder_path)
        created_files += c
        modified_files += m
        deleted_files += d
        total_size += size
    ...
    # decky_plugin.logger.info summary in a single line
    decky_plugin.logger.info("")
    decky_plugin.logger.info(f"Created: {created_files}, Modified: {modified_files}, Deleted: {deleted_files}")
    decky_plugin.logger.info(f"Copied: {format_size(total_size)} in {display_time} seconds ({format_speed(speed)})")
...
def copyFromRemote():
    ...
    for entry_name, details in data.items():
        ...
        inclusions = details.get('inclusions', ['*'])
        exclusions = set(details.get('exclusions', []))
        c, m, d, size = process_entry(entry_name, inclusions, exclusions, folder_path, remote_dir + "/" + entry_name)
        created_files += c
        modified_files += m
        deleted_files += d
        total_size += size
